...
async function generateVapidJWT(endpoint: string, vapidPrivateKey: string, vapidPublicKey: string): Promise<string> {
  ...
  const subject = 'mailto:contact@futureu.app';
...
async function hkdf(salt: Uint8Array, ikm: Uint8Array, info: Uint8Array, length: number): Promise<Uint8Array> {
  ...
  const saltKey = await crypto.subtle.importKey(
    'raw', saltKeyBytes.buffer as ArrayBuffer, { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']
  );
...
async function encryptPayload(
  ...
  const aesKey = await crypto.subtle.importKey(
    'raw', contentEncryptionKey.buffer as ArrayBuffer, { name: 'AES-GCM' }, false, ['encrypt']
  );

  const ciphertext = new Uint8Array(
    await crypto.subtle.encrypt(
      { name: 'AES-GCM', iv: nonce.buffer as ArrayBuffer }, aesKey, paddedPayload.buffer as ArrayBuffer
    )
  );
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data, error: claimsError } = await authClient.auth.getClaims(authHeader.replace('Bearer ', ''));
    ...
    // Users can only send push notifications to themselves
    if (callerId !== user_id) {
      return new Response(
    ...
    // Use service role for fetching subscriptions (bypasses RLS)
    ...
    for (const subscription of subscriptions) {
      try {
        ...
        const body = encrypted.buffer.slice(
          encrypted.byteOffset, encrypted.byteOffset + encrypted.byteLength
        ) as ArrayBuffer;
